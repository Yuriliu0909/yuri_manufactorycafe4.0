...
postData('/yuricafe/services/service-main.php',{req:'menu'} ).then((response)=>{
    ...
    response.forEach((res)=>{
        contents += '<div class="col mb-5">\n' +
            ...
            '                                <!-- Product name-->\n' +
            '                                <h5 class="fw-bolder" >'+res['name']+'</script></h5></script></input>Price:$'+res['price'] +'\n' +
            '                            </div>\n' +
            '                        </div>\n' +
            '                        <!-- Product actions-->\n' +
            '                        <div class="card-footer p-4 pt-0 border-top-0 bg-transparent">\n' +
            '                            <div class="text-center"><button onclick="addItemToCart('+res['id']+','+res['price']+')" class="btn btn-outline-dark mt-auto" >Add to Cart</bi></div>\n' +
            '                        </div>\n' +
            '                    </div>\n' +
            '                </div>';
